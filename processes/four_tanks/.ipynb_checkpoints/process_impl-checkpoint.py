...
def differential_equations(t, initial_conditions, constants, actions, adaptive_vars):
    ...
    h1, h2, h3, h4 = initial_conditions
    q1, q2 = actions["q1"], actions["q2"]
    k1, k2 = adaptive_vars["k1"], adaptive_vars["k2"]
...
def algebraic_equations(vars_pvi, constants, actions, state):
    ...
    """
    state["h1"] = vars_pvi["h1"]
    state["h2"] = vars_pvi["h2"]
    state["h3"] = vars_pvi["h3"]
    state["h4"] = vars_pvi["h4"]
    state["k1"] = state["k1"]
    state["k2"] = state["k2"]
...
config = {
    ...
    "constants": {
        ...
        "max_steps": 75,  # duração do episódio
